...
export default function CadastrarCliente() {
    ...
    const handleFormSubmit = async (data: LoginFormSchema) => {
        const { nome, email, telefone, cpf_cnpj } = data;
                ...
                <Card className="p-10 drop-shadow-xl rounded-xl">
                    ...
                    <div className="pb-16 grid grid-cols-1 sm:grid-cols-2 gap-10">
                        <div className="flex flex-col space-y-1.5">
                            <input type="text" className={`border-b-2 focus:border-b-2 ${errors.nome ? 'border-red-500' : ''}`} placeholder="Nome" {...register('nome')} />
                            {errors.nome && <div className="text-red-500">{errors.nome.message}</div>}
                        </div>
                        <div className="flex flex-col space-y-1.5">
                            <input type="text" className={`border-b-2 focus:border-b-2 ${errors.cpf_cnpj ? 'border-red-500' : ''}`} placeholder="CPF/CNPJ"  {...register('cpf_cnpj')} />
                            {errors.cpf_cnpj && <div className="text-red-500">{errors.cpf_cnpj.message}</div>}
                        </div>
                        <div className="flex flex-col space-y-1.5">
                            <input type="text" className={`border-b-2 focus:border-b-2 ${errors.email ? 'border-red-500' : ''}`} placeholder="Email"  {...register('email')} />
                            {errors.email && <div className="text-red-500">{errors.email.message}</div>}
                        </div>
                        <div className="flex flex-col space-y-1.5">
                            <input type="text" className={`border-b-2 focus:border-b-2 ${errors.telefone ? 'border-red-500' : ''}`} placeholder="Telefone"  {...register('telefone')} />
                            {errors.telefone && <div className="text-red-500">{errors.telefone.message}</div>}
                        </div>
                    </div>
                    <div className="flex justify-center">
                        <button className="w-full bg-blue-500 hover:bg-blue-700 text-white hover:text-white font-bold py-2 px-4 rounded" type="submit">CADASTRAR CLIENTE</button>
                    </div>
                </Card>
            </div>
        </form>
